...
export type ImageBuilderItemParamsFormProps = {
  ...
  onChange: (values: ImageItemParameters, index: number) => void;
};

export const ImageBuilderItemParamsForm: React.FC<ImageBuilderItemParamsFormProps> = React.memo(
  ({ values, onChange, index }) => {
    ...
    useEffect(() => {
      if (JSON.stringify(watchedValues) !== JSON.stringify(values)) {
        ...
        onChange(copiedValues, index);
      }
    }, [watchedValues, onChange, index, values]);
